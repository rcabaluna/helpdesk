
<div class="form-row">
    <div class="form-group col-md-12">
        <label for="txteqptsftware">Equipment or Software needed <span class="text-danger">*</span></label>
        <div class="checkbox">
            <input id="checkbox1" name="confictrequested" type="checkbox" value="Epson Projector">
            <label for="checkbox1">Epson Projector</label>
        </div>
        <div class="checkbox">
            <input id="checkbox2" name="confictrequested" type="checkbox" value="Logitech Video Conference device">
            <label for="checkbox2">Logitech Video Conference device</label>
        </div>
    </div>
</div> 